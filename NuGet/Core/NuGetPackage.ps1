...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0 -Or $IsDebug) {
	$packageTask = Create-Process .\NuGet.exe ("pack Package.nuspec -Symbols -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit(30000)
